...
class DocSummarizer():
    ...
    def valid_token(self, token):
        return (
            ...
            not token in self.en_most_common and # token is not one of english's most used words
            len(token) > 2
        )
    ...
    async def summarize(self, rss_data: RSSData):
        ...
        futures = [loop.run_in_executor(self.executor, self.query_wikipedia, word) for word in filtered_words]
